...
def CMnetEncoder(N):
    ...
    h2 = Dense(N*2, activation='tanh', name='Dense2')(h1)
    h2 = BatchNormalization(name='DenseBN2')(h2)

    h3 = Dense(N*2, activation='tanh', name='Dense3')(h2)
    h3 = BatchNormalization(name='DenseBN3')(h3)

    h4 = Dense(N*2, activation='tanh', name='Dense4')(h3)
    h4 = BatchNormalization(name='DenseBN4')(h4)


    h6 = Dense(N, activation='tanh', name='Dense6')(h4)
    h6 = BatchNormalization(name='DenseBN6')(h6)

    enc_out = Lambda(lambda x: x**3, name='encoder_output')(h6)

    return Model(inputs=[enc_in], outputs=[enc_out], name="CMnet_Encoder")    # 包含从 enc_in 到 enc_out 的计算的所有网络层
...
def CMnetDecoder(N):
    ...
    h8 = Dense(N*2, activation='tanh', name='Dense8')(h7)
    h8 = BatchNormalization(name='DenseBN8')(h8)

    h9 = Dense(N*2, activation='tanh', name='Dense9')(h8)
    ...
    return Model(inputs=[dec_in], outputs=[dec_out], name="CMnet_Decoder")
...
def CMnetAutoEncoder(N, enc, dec):
    ...
    # taking ifft of encoder output - used to minimize PAPR(CM)
